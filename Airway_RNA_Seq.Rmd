...
1.  **`rowData(airway)`**:
    ...
    -   This function retrieves the metadata associated with each row (typically representing genes) in the `airway`dataset. This metadata might include gene identifiers, gene symbols, annotations, and other relevant information about each feature (e.g., genes or transcripts) in the dataset.
...
**Method for Direct Normalization Using `DESeq()`**
...
Then, we plot the dispersion estimate
